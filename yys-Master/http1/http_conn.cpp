...
void http_conn::init()
{
    ...
    bytes_to_send = 0;
    bytes_have_send = 0;
    m_check_state = CHECK_STATE_REQUESTLINE;//其实状态，解析请求行
    m_linger = false;//http请求是否保持连接
    m_method = GET;//GET请求
    m_url = 0;//目标文件名
    m_version = 0;//版本号
    m_content_length = 0;//HTTP请求的消息体长度
    m_host = 0;//主机名
    m_start_line = 0;//当前正在解析的行的起始位置
    m_check_idx = 0;//当前正在分析的字符在读缓冲区的位置
    m_read_idx = 0;//下一个需要读的位置的起始索引
    m_write_idx = 0;//写缓冲区中代发送的字节数
    cgi = 0;//是否启用POST
    m_state = 0;//读还是写，0为读
    timer_flag = 0;
    improv = 0;
...
//从状态机，用于分析出一行内容
//返回值是行的读取状态，有LINE_OK,LINE_BAD,LINE_OPEN
...
bool http_conn::read_once()
{
    ...
    else 
    {
        ...
        while( true)
        {
            ...
            if( bytes_read == -1)
            {
                if(errno == EAGAIN || errno == EWOULDBLOCK)//没有数据可以读了,下次再来
                    break;
                return false;
            }
...
//解析http请求行，获得请求方法，目标url及其http版本号
// \t是tab的意思
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    
    m_url = strpbrk(text, " \t");//遇到空格或者\t就返回

    if(!m_url)
        return BAD_REQUEST;
    *m_url ++ ='\0';//把前面的截取下来，指向新的下一个起点
    char *method = text;//截取下来的去判断一下方法
    ...
    else if (strcasecmp(method,"POST") == 0)
    {
        ...
        return BAD_REQUEST;
    m_url += strspn(m_url, " \t");//跳过新起点前面的空格或者\t
    m_version = strpbrk(m_url, " \t");//然后跳过中间的请求资源,指向http类型
    if(!m_version)
        return BAD_REQUEST;
    *m_version++ ='\0';
    m_version += strspn(m_version, " \t");//跳过前面的空格或者\t
    if(strcasecmp(m_version,"HTTP/1.1") != 0)//与"HTTP/1.1长度是否相等" 比较的是长度
        return BAD_REQUEST;
    if(strncasecmp(m_url,"http://",7) == 0)//前七个字符是不是"http://"  比较的是内容
    {
        m_url += 7;
    ...
    return NO_REQUEST;//请求不完整，需要继续处理请求数据
}
...
//当得到一个完整的、正确的HTTP请求时，我们就分析目标文件的属性。如果目标文件存在、对所有
...
bool http_conn::write()
{
    ...
    while (1)
    {
        ...
        bytes_have_send += temp;//发送的字节数
        bytes_to_send -= temp;//剩余字节数
